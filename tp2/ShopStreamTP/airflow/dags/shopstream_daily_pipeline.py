...
from airflow.providers.snowflake.operators.snowflake import Snowf
...
dag = DAG(
    ...
    description='Pipeline quotidien ShopStream : PostgreSQL vers' 
    schedule_interval='0 2 * * *',  # Tous les jours à 2h du mati
    catchup=False,
...
def extract_postgres_to_s3(**context):
    ...
    import subprocess
    execution_date = context['ds']  # Date d'exécution (YYYY-MM-D
    ...
    result = subprocess.run(
        ['python', 'D:\Desktop\semestre 7\tp amamou2\ShopStreamTP\scripts\export_to_s3']
        capture_output=True,
        text=True
    )
    
    if result.returncode != 0:
        raise Exception(f"Erreur lors de l'export : {result.stder
...
def run_dbt_models(**context):
    ...
    # MODIFIEZ LE CHEMIN SELON VOTRE INSTALLATION
    result = subprocess.run(
        ['dbt', 'run', '--project-dir', '/chemin/vers/ShopStreamT
        capture_output=True,
        text=True
    )
...
task_wait_s3_users = S3KeySensor(
    ...
    bucket_name='shopstream-datalake-votreprenom',  # MODIFIEZ av
...
task_load_staging_users = SnowflakeOperator(
    ...
    sql="""
        ...
        COPY INTO stg_users (id, email, first_name, last_name, co
...
task_load_staging_orders = SnowflakeOperator(
    ...
    sql="""
        ...
        COPY INTO stg_orders (id, user_id, created_at, total_amou
...
[task_load_staging_users, task_load_staging_orders] >> task_dbt_r
